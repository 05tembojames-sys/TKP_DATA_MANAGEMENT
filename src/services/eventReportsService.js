...
class EventReportsService {
  ...
  async generateReport(config) {
    try {
      ...
      if (config.program === "child-assessment") {
      ...
      } else if (config.program === "child-overview") {
        const result = await FormService.getForms("child-overview", 1000);
        if (result.success) {
          forms = result.forms;
        }
      } else if (config.program === "initial-assessment") {
        const result = await FormService.getForms("initial-assessment", 1000);
        if (result.success) {
          forms = result.forms;
        }
      } else if (config.program === "medical-intake") {
        const result = await FormService.getForms("medical-intake", 1000);
        if (result.success) {
          forms = result.forms;
        }
      } else if (config.program === "academics-literacy") {
        const result = await FormService.getForms("academics-literacy", 1000);
        if (result.success) {
          forms = result.forms;
        }
      } else if (config.program === "psychological-assessment") {
      ...
      } else if (config.program === "life-skills-survey") {
        const result = await FormService.getForms("life-skills-survey", 1000);
        if (result.success) {
          forms = result.forms;
        }
      } else if (config.program === "birth-delivery") {
        const result = await FormService.getForms("birth-delivery", 1000);
        if (result.success) {
          forms = result.forms;
        }
      } else if (config.program === "care-plan-summary") {
        const result = await FormService.getForms("care-plan-summary", 1000);
        if (result.success) {
          forms = result.forms;
        }
      } else if (config.program === "care-plan-baby") {
        const result = await FormService.getForms("care-plan-baby", 1000);
        if (result.success) {
          forms = result.forms;
        }
      } else if (config.program === "care-plan-ongoing-life-skills") {
        const result = await FormService.getForms(
        ...
        const [referralResult, overviewResult, assessmentResult] =
          await Promise.all([
  ...
  buildRows(forms, selectedElements, rowDimensions, aggregationType) {
    ...
    Object.keys(grouped).forEach((groupKey) => {
      ...
      selectedElements.forEach((elementId) => {
        const value = this.aggregateElementData(
          groupForms,
          elementId,
          aggregationType
        );
  ...
  groupForms(forms, dimensions) {
    ...
    forms.forEach((form) => {
      ...
      if (dimensions.includes("period")) {
        const formDate = this.getFormDate(form);
        if (formDate) {
          const monthName = formDate.toLocaleDateString("en-US", {
            month: "long",
            year: "numeric",
          });
          groupKey = monthName;
    ...
    forms.forEach((form) => {
      ...
      switch (elementId) {
        ...
        case "gender":
          ...
          break;
        case "pregnant":
          // Check various fields that might indicate pregnancy status
          value =
            form.pregnant === "Y" || form.pregnancyLength || form.visitedClinic
              ? 1
              : 0;
          break;
        case "status":
          value =
            form.status === "completed" ? 1 : form.status === "draft" ? 0 : 0.5;
          break;
        case "attendance":
  ...
  getFormTypeDisplay(formType) {
    const types = {
      ...
      "care-plan-ongoing-life-skills": "Ongoing Life Skills",
